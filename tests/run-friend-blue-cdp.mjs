...
try {
  ...
  await evaluate(client, await readFile(new URL("./friend-blue.browser.js", import.meta.url), "utf8"));
  const report = await evaluate(
    client,
    `window.__runMisakaFriendBlue(${JSON.stringify({ repeats })})`,
    true,
  );
  ...
  await evaluate(client, `(() => {
    ...
    window.__misakaTestLifecycle?.dispose?.("friend-suite-complete");
