...
- **Misaka iPad Guard v0.3.7**：在 WebContent 被 Jetsam 回收前跨站释放网页进程，返回 BC 后先等待 5 秒供插件加载，再使用 Tampermonkey 私有存储中的御坂密码调用 BC 原生登录并恢复原房间，同时保存本地生命周期日志；三套脚本不在进房初始化时写入聊天框，命令反馈仍采用统一的状态顺序、反馈句式与蓝色。
