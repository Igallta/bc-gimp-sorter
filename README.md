...
- **Misaka iPad Guard v0.3.1**：在 WebContent 被 Jetsam 回收前跨站释放网页进程，返回 BC 后先等待 5 秒供插件加载，再使用 Tampermonkey 私有存储中的御坂密码调用 BC 原生登录并恢复原房间，同时保存本地生命周期日志；本地提示颜色与 MisakaChat 保持一致，不会修改 MisakaChat 或 GimpSorter 的业务逻辑。
