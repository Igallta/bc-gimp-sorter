// MisakaChat v2.10.8 - BC 御坂自动回复系统
// 模块分区:
//   [Config]      L15-55   配置 + 状态
...
格式:{"intent":"action|chat|clarify","usedPendingClarification":false,"needsCatalog":true,"goal":"最终目标","constraints":{"noMove":false,"noAdd":false,"replaceExisting":false,"noStack":false,"preserveParts":[]},"operations":[{"types":["itemadd"],"targets":[123],"parts":[],"assets":["LowCage"]}],"question":""}
房间名单:${roster}
...
当前实时道具高于历史对话。调整/收紧/替换现有道具时，以这里是否存在为准；存在则应规划 action，不存在才 clarify。`;
    ...
    try {
      ...
      return { intent: "clarify", usedPendingClarification: false, needsCatalog: false, operations: [], question: "我没听明白要做什么，能再说具体一点吗？", failed: true };
    }
  }
  ...
  function filterCommandsByPlan(plan, commands) {
    ...
    // 若计划要求添加一个已经选定的精确 Asset，却没有任何对应 ITEMADD，
    // 不能只执行“删除旧道具”这种半套替换。把本操作的道具命令全部退回，
    // 让上层进入强制纠错；重试仍不正确则零执行。
    for (const operation of operations) {
      const requiredAssets = (Array.isArray(operation.assets) ? operation.assets : [])
        .map(name => String(name).toLowerCase());
      if (requiredAssets.length === 0 || !operation.types?.includes("itemadd")) continue;
      const operationTargets = (operation.targets || []).map(Number);
      const hasRequiredAdd = allowed.some(cmd => {
        if (cmd.type !== "itemadd" || !operationTargets.includes(commandPrimaryTarget(cmd))) return false;
        const mapping = findItemAsset(cmd.item, actionTargetCharacter(commandPrimaryTarget(cmd)));
        return requiredAssets.includes(String(mapping?.asset || cmd.item || "").toLowerCase());
      });
      if (hasRequiredAdd) continue;
      for (let i = allowed.length - 1; i >= 0; i--) {
        const cmd = allowed[i];
        if (!["itemadd", "itemdel", "itemdelall", "itemset", "itemcolor"].includes(cmd.type)) continue;
        if (!operationTargets.includes(commandPrimaryTarget(cmd))) continue;
        allowed.splice(i, 1);
        rejected.push({ cmd, reason: "planned-asset-missing" });
      }
    }
  ...
  async function verifyActionOutcome(plan, commands, baseline) {
    ...
    if (!plan?.goal) return { satisfied: true, reason: "确定性后置条件与限制均满足", postconditions, constraints };
    const effects = verifiedEffectSummary(postconditions);
    const finalFacts = buildCurrentAppearanceFacts(plan);
    const prompt = `你是 BC 操作计划的语义映射验收器。只输出一行严格 JSON，不要 markdown，不要回复用户。
确定性执行层已经验证下列道具、属性、颜色和限制确实生效。你只判断这些“已验证效果”的语义是否完整对应用户 goal。
BC 的实际 Asset、Group、Typed/Modular 选项名、SetPose 和 Effect 是权威定义。不得自行追加 goal 没有要求的道具、部位或步骤；不得用现实世界绑法臆测覆盖游戏内选项定义。
如果已验证效果是该命名姿势或道具状态在 BC 中的标准实现，应判 true；如果只是相近状态、漏掉 goal 明确要求的子任务，才判 false。
没有产生道具后置条件的移动、表情、快照或复制操作，以“实际执行指令”和最终实时状态辅助判断；不得仅因已验证效果数组为空就判失败。
格式:{"satisfied":true,"reason":"简短原因"}
计划:${JSON.stringify({ goal: plan.goal, constraints: plan.constraints || {}, operations: plan.operations || [] })}
实际执行指令:${JSON.stringify(commands)}
已验证效果:${JSON.stringify(effects)}
${finalFacts}`;
    const result = await callLLM(prompt, [{ role: "user", content: "只验收目标与已验证效果的语义映射。" }], { thinking: true, maxTokens: 1024 });
    try {
      const match = String(result || "").match(/\{[\s\S]*\}/);
  ...
  async function handleReply(senderNum, senderName, content) {
    ...
    try {
      ...
      if (executableCommands.length > 0) {
        ...
        if (commandResult.rolledBack && commandResult.rollbackReason !== "outcome-unsatisfied") finalReply = "这次没弄成，我已经恢复原样了。";
        if ((commandResult.failures || []).length === 0) {
