  ...
  function parseActionCommands(reply) {
    ...
    const cleaned = String(reply || "").replace(/\[([A-Z]+):([^\]]*)\]/gi, (raw, rawType, body) => {
      ...
      if (type === "MEMSEARCH" && body.trim()) cmd = { type: "memsearch", query: body.trim() };
        ...
        cmd = { type: "itemadd", memberNumber: mn, item: parts[1], part: parts[2] || "", color: parts.slice(3).join(":") };
    ...
    if (constraints.replaceExisting || constraints.noStack) {
      for (let i = allowed.length - 1; i >= 0; i--) {
        ...
        const hasPriorDelete = allowed.slice(0, i).some(prev =>
          ...
          (prev.type === "itemdel" && prev.memberNumber === cmd.memberNumber && (!cmd.part || !prev.part || prev.part === cmd.part))
        );
        if (!hasPriorDelete) {
          allowed.splice(i, 1);
...
命名姿势、替换、不要叠加、不要新增等语义必须完整满足；只完成一部分时 satisfied=false。
