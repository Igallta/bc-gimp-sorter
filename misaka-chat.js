...
当前实时道具高于历史对话。调整/收紧/替换现有道具时，以这里是否存在为准；存在则应规划 action，不存在才 clarify。`;
  ...
  async function verifyActionOutcome(plan, commands) {
    ...
    if (/驷马缚|hogtie/i.test(plan.goal)) {
      ...
      // BC 的 ItemArms HempRope/Hogtied（以及同义 SimpleHogtie）本身就是完整的
      // Hogtie 姿势设置。执行成功后不再交给自由语义验收器追加“必须另有腿绳”之类
      // 游戏规则中并不存在的条件，避免把真实成功误判后回滚。
      return { satisfied: true, reason: "已设置有效的 Hogtie/驷马缚样式" };
    }
